...
// @version      10
// @description  Скрипт, который меняет стиль Twitblit под TBL.
// @author       G02m0n1k
...
function insertBlocks() {
    ...
    const newWidth = Math.round(currentWidth * 1.18);
...
function removeBlur() {
    ...
    elements.forEach(element => {
        ...
        if (style.includes('filter: blur(3px); pointer-events: none; flex-shrink: 0;')) {
            let newStyle = style.replace('filter: blur(3px); pointer-events: none; flex-shrink: 0;', '');
        ...
        if (style.includes('cursor: not-allowed; user-select: none;')) {
            let newStyle = style.replace('cursor: not-allowed; user-select: none;', '');
            newStyle = newStyle.trim();
